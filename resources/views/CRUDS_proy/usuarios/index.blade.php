...
<div class="container mt-5" style="max-width: 900px;">
    <div class="card p-4 shadow-lg" style="border: 1px solid #1e3a8a; border-radius: 15px;">
        <h2 class="text-center mb-4" style="color: #0d47a1;">Lista de Usuarios</h2>
        @if (session('success'))
            <div class="alert alert-success">
                {{ session('success') }}
            </div>
        @endif
        <button type="button" onclick="window.location.href='{{ route('usuarios.create') }}'" class="btn btn-success mb-3" style="background-color: #388e3c; color: white; border-radius: 8px; padding: 12px 24px; font-weight: bold; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); transition: background-color 0.3s ease;">
            Añadir Usuario
        </button>
            ...
            <tbody>
                @foreach ($usuarios as $usuario)
                    <tr id="row-{{ $usuario->id }}">
                        ...
                        <td>{{ $usuario->role }}</td>
                        <td class="d-flex">
                            <button type="button" onclick="window.location.href='{{ route('usuarios.edit', $usuario->id) }}'" class="btn btn-sm" style="background-color: #0d508f; color: white; border-radius: 8px; margin-right: 10px; padding: 8px 16px; font-weight: bold; transition: background-color 0.3s ease;">
                                Editar
                            </button>
                            
                            <form action="{{ route('usuarios.destroy', $usuario->id) }}" method="POST" class="delete-form">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger" style="background-color: #ff4444; color: white; border-radius: 8px; padding: 8px 16px; font-weight: bold; transition: background-color 0.3s ease;" data-id="{{ $usuario->id }}">
                                    Eliminar
                                </button>
                            </form>
                        </td>
